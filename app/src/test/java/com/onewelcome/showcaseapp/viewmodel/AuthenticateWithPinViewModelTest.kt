...
class AuthenticateWithPinViewModelTest {
  ...
  private lateinit var viewModel: AuthenticateWithPinViewModel

  @Before
  fun setup() {
    hiltRule.inject()
    viewModel = AuthenticateWithPinViewModel(pinAuthenticationRequestHandler)
  }
  ...
  fun `When Submit event is sent, Then accept authentication request should be triggered`() {
    ...
    viewModel = AuthenticateWithPinViewModel(spyCreatePinRequestHandler)
