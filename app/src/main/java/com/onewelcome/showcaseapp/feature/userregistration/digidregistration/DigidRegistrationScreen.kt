...
import com.onewelcome.showcaseapp.feature.userregistration.digidregistration.DigidRegistrationViewModel.NavigationEvent
import com.onewelcome.showcaseapp.feature.userregistration.digidregistration.DigidRegistrationViewModel.State
import com.onewelcome.showcaseapp.feature.userregistration.digidregistration.DigidRegistrationViewModel.UiEvent
import com.onewelcome.showcaseapp.navigation.Screens
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.emptyFlow

@Composable
fun DigidRegistrationScreen(
    navController: NavController,
    pinNavController: NavController
) {
    val parentEntry = remember(navController) {
        navController.getBackStackEntry(Screens.DigidRegistration.route)
    }
    val viewModel: DigidRegistrationViewModel = hiltViewModel(parentEntry)
    DigidRegistrationScreenContent(
...
@Composable
fun DigidRegistrationScreenContent(
    uiState: State,
    onNavigateBack: () -> Unit,
    ...
    SdkFeatureScreen(
        title = stringResource(R.string.title_digid_registration),
        onNavigateBack = onNavigateBack,
        description = {
            ShowcaseFeatureDescription(
                description = stringResource(R.string.digid_registration_description),
    ...
    LaunchedEffect(Unit) {
        navigationEvents.collect { event ->
            when (event) {
                ...
                is NavigationEvent.ToDigidRegistrationScreen -> { /* no-op: intentionally do nothing */ }
                is NavigationEvent.ToTwoStepVerificationScreen -> { /* no-op: intentionally do nothing */ }
                is NavigationEvent.OpenDigidUrl -> {
...
private fun Preview() {
    ...
    val digidProviders = setOf(
        object : OneginiIdentityProvider {
            override val id: String
                get() = "poc_digid_two_step"
            override val name: String
                get() = "DigiD Registration"
    ...
    DigidRegistrationScreenContent(
        uiState = State(identityProviders = digidProviders, isSdkInitialized = true),
        onNavigateBack = {},
        onEvent = {},
        onNavigateToPinScreen = {},
        onNavigateToTwoStepInputScreen = {},
