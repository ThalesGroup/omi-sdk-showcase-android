...
class TwoStepRegistrationViewModel @Inject constructor(
  ...
  fun onEvent(event: UiEvent) {
    when (event) {
      is UiEvent.StartTwoStepRegistration -> startRegistration()
      ...
      is UiEvent.SubmitOptionalData -> subnitOptionalData(event.optionalData)
      is UiEvent.EnterTwoStepInput -> enterOptionalData()
    }
  }

  private fun subnitOptionalData(optionalData: String) {
    twoStepRegistrationRequestHandler.optionalData = optionalData
    onEvent(UiEvent.StartTwoStepRegistration)
  }
  ...
  private fun cancelRegistration() {
    viewModelScope.launch {
      ...
      uiState = uiState.copy(
        ...
        result = Err(Exception("Registration canceled by user"))
      )
    }
  }
  ...
  private fun listenForTwoStepInputNavigationEvent() {
    viewModelScope.launch {
      twoStepRegistrationRequestHandler.startTwoStepInputFlow.collect { inputData ->
        ...
        _navigationEvents.send(NavigationEvent.ToTwoStepVerficationScreen)
      }
    }
  }
  ...
  private suspend fun handleSuccess(pair: Pair<UserProfile, CustomInfo?>) {
    _navigationEvents.trySend(NavigationEvent.ToTwoStepRegistraionScreen)
  ...
  sealed interface UiEvent {
    data object StartTwoStepRegistration : UiEvent
    data object EnterTwoStepInput : UiEvent
  ...
  sealed interface NavigationEvent {
    data object ToTwoStepRegistraionScreen : NavigationEvent
    data object ToPinScreen : NavigationEvent
    data object ToTwoStepInputScreen : NavigationEvent
    data object ToTwoStepVerficationScreen : NavigationEvent
  }
}
