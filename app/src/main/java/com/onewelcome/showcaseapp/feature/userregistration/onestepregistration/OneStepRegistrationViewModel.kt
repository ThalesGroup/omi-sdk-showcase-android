...
class OneStepRegistrationViewModel @Inject constructor(
  ...
  private var pinListenerJob: Job? = null
  private var qrCodeListenerJob: Job? = null
  ...
  private fun listenForPinScreenNavigationEvent() {
    pinListenerJob?.cancel()
    pinListenerJob = viewModelScope.launch {
  ...
  private fun listenForQrCodeNavigationEvent() {
    qrCodeListenerJob = viewModelScope.launch {
      qrCodeRegistrationAction.awaitCallback()
